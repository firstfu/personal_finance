// ============================================================================
// MARK: - CSVExporter.swift
// 模組：Helpers
//
// 功能說明：
//   這個檔案提供交易紀錄的 CSV 匯出功能，可將交易資料轉換為
//   逗號分隔格式的字串或暫存檔案，供使用者匯出與分享。
//
// 主要職責：
//   - export(transactions:)：將交易陣列轉換為 CSV 格式字串
//     （欄位：日期、類型、分類、帳戶、金額、備註）
//   - exportToFile(transactions:)：將 CSV 字串寫入暫存目錄檔案並回傳 URL
//   - 自動過濾示範資料（isDemoData）、依日期降序排列
//
// 注意事項：
//   - 備註欄位中的半形逗號會被替換為全形逗號，以避免破壞 CSV 格式
//   - 匯出檔名包含當前日期（繁體中文格式）
//   - 檔案寫入暫存目錄（temporaryDirectory），系統可能自動清理
// ============================================================================
